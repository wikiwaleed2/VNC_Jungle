...
vncClientId
vncServer::AddClient(VSocketBase *socket, BOOL reverse, BOOL shared)
{
...
vncServer::SockConnect(BOOL On)
{
	...
	if (On)
	{
		...
		if (m_jingleConn == NULL) {
			m_jingleConn = new vncJingleConnect();
			if (m_jingleConn == NULL) {
				return FALSE;
			}
			m_jingleConn->Init(this, GetXmppUsername(), GetXmppPassword());
		}
