...
vncEncoder::LogStats()
{
	vnclog.Print(LL_INTINFO, VNCLOG("%s c: data=%d, overhead=%d, "
									"encoded=%d, sent=%d\n"),
				 GetEncodingName(),
...
inline UINT
vncEncoder::EncodeRect(BYTE *source, VSocketBase *outConn, BYTE *dest, const RECT &rect, int offsetx, int offsety)
{
...
BOOL
vncEncoder::SendEmptyCursorShape(VSocketBase *outConn)
{
...
BOOL
vncEncoder::SendCursorShape(VSocketBase *outConn, vncDesktop *desktop)
{
...
BOOL
vncEncoder::SendXCursorShape(VSocketBase *outConn, BYTE *mask,
...
BOOL
vncEncoder::SendRichCursorShape(VSocketBase *outConn, BYTE *mbits, BYTE *cbits,
								int xhot, int yhot, int width, int height)
{
	rfbFramebufferUpdateRectHeader hdr;
	hdr.r.x = Swap16IfLE(xhot);
