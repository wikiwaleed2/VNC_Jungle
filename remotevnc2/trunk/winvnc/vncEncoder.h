...
public:
	...
	virtual UINT EncodeRect(BYTE *source, VSocketBase *outConn, BYTE *dest, const RECT &rect, int offsetx, int offsety);
	...
	BOOL SendEmptyCursorShape(VSocketBase *outConn);
	BOOL SendCursorShape(VSocketBase *outConn, vncDesktop *desktop);

protected:
	BOOL SetTranslateFunction();

	// Supporting cursor shape updates
	BOOL SendXCursorShape(VSocketBase *outConn, BYTE *mask, int xhot,int yhot,int width,int height);
	BOOL SendRichCursorShape(VSocketBase *outConn, BYTE *mbits, BYTE *cbits, int xhot,int yhot,int width,int height);
