...
PresencePushTask::ProcessStart() {
  ...
  if (stanza->Attr(QN_TYPE) == STR_UNAVAILABLE) {
    ...
    if (caps != NULL) {
      ...
      if (node == PAGLO_CLIENT_NODE) {
        s.set_is_paglo_client(true);
        s.set_version(ver);
				if (ListContainsToken(exts, "screen-share-server-v1")) {
				  s.set_screenshare_server_capability(true);
				}
				if (ListContainsToken(exts, "screen-share-client-v1")) {
				  s.set_screenshare_client_capability(true);
				}
      }
    }
    ...
    if (nick != NULL) {
      std::string nickname = nick->BodyText();

      s.set_nickname(nickname);
    }
