...
PresenceOutTask::TranslateStatus(const Status & s) {
  ...
  if (!s.available()) {
    ...
    if (s.know_capabilities() && s.is_paglo_client()) {
      std::string caps;
			if (s.screenshare_server_capability()) {
	      caps += "screen-share-server-v1";
			}
			if (s.screenshare_client_capability()) {
				if (!caps.empty()) {
					caps += " ";
				}
	      caps += "screen-client-server-v1";
			}
      result->AddElement(new XmlElement(QN_CAPS_C, true));
      result->AddAttr(QN_NODE, PAGLO_CLIENT_NODE, 1);
    ...
    // Put the delay mark on the presence according to JEP-0091
    {
      ...
      result->AddAttr(kQnStamp, output, 1);
		...
		result->AddElement(new XmlElement(kQnNickname, true));
    result->AddText(s.nickname(), 1);
