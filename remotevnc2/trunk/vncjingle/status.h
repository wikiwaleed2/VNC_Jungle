...
#define PAGLO_CLIENT_NODE "http://www.paglo.com/xmpp/client/caps"
...
public:
  Status() :
    ...
    available_(false),
    e_code_(0),
    screenshare_server_capability_(false),
    screenshare_client_capability_(false),
    know_capabilities_(false),
    is_paglo_client_(false),
  ...
  bool screenshare_server_capability() const { return screenshare_server_capability_; }
  bool screenshare_client_capability() const { return screenshare_client_capability_; }
  bool is_paglo_client() const { return is_paglo_client_; }
  ...
  void set_screenshare_server_capability(bool f) { screenshare_server_capability_ = f; }
  void set_screenshare_client_capability(bool f) { screenshare_client_capability_ = f; }
  void set_is_paglo_client(bool f) { is_paglo_client_ = f; }
  ...
  void UpdateWith(const Status & new_value) {
    if (!new_value.know_capabilities()) {
       ...
       bool i = is_paglo_client();
       std::string v = version();

       *this = new_value;

       set_know_capabilities(k);
       set_is_paglo_client(i);
       set_version(v);
    }
    else {
      *this = new_value;
...
private:
  ...
  bool screenshare_server_capability_;
  bool screenshare_client_capability_;
  bool is_paglo_client_;
  std::string version_;
