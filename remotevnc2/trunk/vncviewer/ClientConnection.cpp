...
void ClientConnection::Init(VNCviewerApp *pApp)
{
	...
	m_hPalette = NULL;
	m_passwdSet = false;
	m_jingleConnection = NULL;
	m_jingleSock = NULL;
	m_successfulLogin = false;
...
void ClientConnection::DoXmppLogin(bool forceLoginPrompt)
{
	// Get the user's login credentials and initiate the login process.
	std::string username, password;

	LoadXmppLogin(username, password);
	if (forceLoginPrompt || username.empty() || password.empty()) {
		XmppLoginDialog logindlg;
		logindlg.m_username = username;
		logindlg.m_password = password;
		if (!logindlg.DoDialog()) {
			throw QuietException("User Cancelled");
		}
		username = logindlg.m_username;
		password = logindlg.m_password;
		std::string save_password;
		if (!logindlg.m_remember_login) {
			save_password = "";
		} else {
			save_password = password;
		}
		SaveXmppLogin(username, save_password);
	}
	
	m_logged_in_as = username;
	m_jingleConnection->Login(username, password);
}

void ClientConnection::OnLoginComplete(bool Connected)
{
	if (Connected) {
		m_successfulLogin = true;
	} else {
		if (!m_successfulLogin) {
			/*
			 * Login failed. Reset the saved credential name to force the
			 * user to be prompted again.
			 */
			std::string username, password;
			LoadXmppLogin(username, password);
			SaveXmppLogin(username, "");
			DoXmppLogin();
		} else {
			// Disconnected.
		}
...
void ClientConnection::Run()
{
  // Get the name of this computer.
	...
	// Create the connection to the XMPP server so we can get a list of available
	// VNC servers.
	m_jingleConnection = new VncJingleConnection(false, desktop_name);
	m_jingleConnection->SignalLoginComplete.connect(this, &ClientConnection::OnLoginComplete);
	...
	if (m_port == -1) {
		...
		if (m_pApp->m_options.m_listening) {
			m_opts.LoadOpt(m_opts.m_display, KEY_VNCVIEWER_HISTORI);
		}
	}
...
HWND ClientConnection::CreateToolbar()
{
  ...
  const int bitmapSize = 22;

  // Create the toolbar.
  HWND hWndToolbar = CreateWindowEx(0, TOOLBARCLASSNAME, NULL, 
    WS_CHILD | TBSTYLE_WRAPABLE | TBSTYLE_LIST | TBSTYLE_FLAT | TBSTYLE_AUTOSIZE, 
    0, 0, 0, 0,
    m_hwnd1, NULL, m_pApp->m_instance, NULL);
  if (hWndToolbar == NULL) {
		return NULL;
  }

	HIMAGELIST ilist = ImageList_Create(22, 22, ILC_COLOR32, numButtons, 0); 
	int buttonID[] = {IDB_TOOL_SETTINGS, IDB_TOOL_STATUS, IDB_TOOL_KEYS, IDB_TOOL_FULLSCREEN, IDB_TOOL_REFRESH,
		IDB_TOOL_KEYS, IDB_TOOL_COPY_FILES, IDB_TOOL_KEYS,
		IDB_TOOL_NEW_CONNECTION, IDB_TOOL_DISCONNECT};
	HBITMAP bmp = NULL; 
	for (int i = 0; i < numButtons; i++) {
		bmp = (HBITMAP)LoadImage(m_pApp->m_instance, MAKEINTRESOURCE(buttonID[i]), IMAGE_BITMAP, 
			bitmapSize, bitmapSize, LR_CREATEDIBSECTION); 
  ...
  int iNew = SendMessage(hWndToolbar, TB_ADDSTRING, 
		(WPARAM)m_pApp->m_instance, (LPARAM)IDS_TOOLBAR); 

  // Initialize button info.
  // IDM_NEW, IDM_OPEN, and IDM_SAVE are application-defined command constants.
  TBBUTTON tbButtons[numButtons] = 
  {
    { 0, IDC_OPTIONBUTTON, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE, {0}, 0, iNew},
    { 1, ID_CONN_ABOUT, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE, {0}, 0, iNew + 1},
    { 2, 0, TBSTATE_ENABLED, TBSTYLE_SEP, {0}, 0, 0},
    { 3, ID_FULLSCREEN, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE, {0}, 0, iNew + 2},
    { 4, ID_REQUEST_REFRESH, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE, {0}, 0, iNew + 3},
    { 5, ID_CONN_CTLALTDEL, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE |  TBSTYLE_DROPDOWN, {0}, 0, iNew + 4},
    { 6, IDD_FILETRANSFER, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE, {0}, 0, iNew + 5},
    { 7, 0, TBSTATE_ENABLED, TBSTYLE_SEP, {0}, 0, 0},
    { 8, ID_NEWCONN, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE, {0}, 0, iNew + 6},
    { 9, ID_DISCONNECT, TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_AUTOSIZE, {0}, 0, iNew + 7},
  };
...
void ClientConnection::SaveXmppLogin(std::string username, std::string password)
{
	HKEY hRegKey;
	TCHAR buf[256];

	RegCreateKey(HKEY_CURRENT_USER, SETTINGS_KEY_NAME, &hRegKey);
	strncpy(buf, username.c_str(), 256);
	RegSetValueEx(hRegKey, "XmppUsername", NULL, REG_SZ, 
		(CONST BYTE *)buf, (_tcslen(buf)+1));			
	strncpy(buf, password.c_str(), 256);
	RegSetValueEx(hRegKey, "XmppPassword", NULL, REG_SZ, 
		(CONST BYTE *)buf, (_tcslen(buf)+1));		
	RegCloseKey(hRegKey);
}

void ClientConnection::LoadXmppLogin(std::string& username, std::string& password)
{
	HKEY hRegKey;
		
	if (RegOpenKey(HKEY_CURRENT_USER, SETTINGS_KEY_NAME, &hRegKey) != ERROR_SUCCESS ) {
		hRegKey = NULL;
	} else {
		TCHAR buf[256];
...
void ClientConnection::Connect()
{
	...
	// Create a data tunnel to our peer.
	m_jingleSock = m_jingleConnection->Connect(m_host_jid);
...
void ClientConnection::SetSocketOptions() {
	...
	if (m_jingleSock->SetSockOpt(IPPROTO_TCP, TCP_NODELAY, (const char *) &nodelayval, sizeof(BOOL)))
...
#define lpnmTB ((LPNMTOOLBAR)lParam)
	...
	{		
		if (lpnm->code == TBN_DROPDOWN) {
			...
			// Get the menu.
			HMENU hMenuLoaded = LoadMenu(pApp->m_instance, MAKEINTRESOURCE(IDR_KEY_POPUP_MENU)); 
	...
	try {
		...
		while (!m_bKillThread) {
			...
			CARD8 msgType;
			{
			  ...
			  int bytes = m_jingleSock->Recv((char *) &msgType, 1, MSG_PEEK);
			  if (bytes == 0) {
...
void ClientConnection::ReadExact(char *inbuf, int wanted)
{
	...
	while (wanted > 0) {
		...
		int bytes = m_jingleSock->Recv(inbuf+offset, wanted, 0);
		if (bytes == 0) throw WarningException("Connection closed.");
		if (bytes == SOCKET_ERROR) {
			int err = m_jingleSock->GetLastError();
...
inline void ClientConnection::WriteExact(char *buf, int bytes)
{
    ...
    while (i < bytes) {
		...
		j = m_jingleSock->Send(buf+i, bytes-i, 0);
