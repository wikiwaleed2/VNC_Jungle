...
#define SETTINGS_KEY_NAME "Software\\Paglo\\RVNCviewer\\Settings"
...
class ClientConnection  : public omni_thread, public sigslot::has_slots<>
{
public:
	...
	int m_port;
  TCHAR m_host[MAX_HOST_NAME_LEN]; // Human readable host name.
	TCHAR m_host_jid[MAX_HOST_NAME_LEN]; // Actual jid to connect to.
	HWND m_hSess;

	/*
	 * Handle vncjingle logins.
	 */
	void DoXmppLogin(bool forceLoginPrompt = false);
	void OnLoginComplete(bool Connected);
	std::string m_logged_in_as;

	VncRosterProxy *GetRoster() { return m_jingleConnection->GetRoster(); }
private:
	...
	VncJingleConnection *m_jingleConnection;
	VncJingleSocket *m_jingleSock;
	bool m_successfulLogin;
	...
	void SaveXmppLogin(std::string username, std::string password);
	void LoadXmppLogin(std::string& username, std::string& password);
