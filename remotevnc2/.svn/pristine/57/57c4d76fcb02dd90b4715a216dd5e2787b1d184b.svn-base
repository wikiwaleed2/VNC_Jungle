...
public:
  ...
  virtual VBool TryAccept(VSocket **new_socket, long ms);
  ...
  virtual VBool SetTimeout(VCard32 secs);

  // I/O routines

  // Send and Read return the number of bytes sent or recieved.
  virtual VInt Send(const char *buff, const VCard bufflen);
  virtual VInt Read(char *buff, const VCard bufflen);

  // SendExact and ReadExact attempt to send and recieve exactly
  // the specified number of bytes.
  virtual VBool SendExact(const char *buff, const VCard bufflen);
  virtual VBool ReadExact(char *buff, const VCard bufflen);
