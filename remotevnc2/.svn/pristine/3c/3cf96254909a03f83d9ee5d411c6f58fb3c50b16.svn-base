...
const char NO_PASSWORD_WARN [] = "WARNING: Running RemoteVNC without setting a password is "
								"a dangerous security risk!\n"
								"Until you set a password, RemoteVNC will not accept incoming connections.";
...
const char NO_PASSWD_NO_OVERRIDE_ERR [] =
								"No password has been set and this machine has been "
								"preconfigured to prevent users from setting their own.\n"
								"You must contact a System Administrator to configure RemoteVNC properly.";
const char NO_PASSWD_NO_LOGON_WARN [] =
								"WARNING: This machine has no Gmail account or default password set.  RemoteVNC will present the "
								"Default Properties dialog now to allow one to be entered.";
const char NO_CURRENT_USER_ERR [] = "The RemoteVNC settings for the current user are unavailable at present.\n"
								"If you have started the service manually, please run the Service Helper as well.";
const char CANNOT_EDIT_DEFAULT_PREFS [] = "You do not have sufficient priviliges to edit the default local RemoteVNC settings.";
...
vncProperties::Show(BOOL show, BOOL usersettings, BOOL passwordfocused)
{
	if (show)
	{
		if (!m_allowproperties)
		{
			...
			MessageBox(NULL, NO_OVERRIDE_ERR, "RemoteVNC Error", MB_OK | MB_ICONEXCLAMATION);
			return;
		}
		...
		if (usersettings) {
			...
			if (!vncService::CurrentUser(username, sizeof(username)))
				return;
			if (strcmp(username, "") == 0) {
				MessageBox(NULL, NO_CURRENT_USER_ERR, "RemoteVNC Error", MB_OK | MB_ICONEXCLAMATION);
				return;
			}
		} else {
			...
			if (!canEditDefaultPrefs) {
				MessageBox(NULL, CANNOT_EDIT_DEFAULT_PREFS, "RemoteVNC Error", MB_OK | MB_ICONEXCLAMATION);
				return;
			}
		}
						  ...
						  LPARAM lParam )
{
	...
	case WM_INITDIALOG:
		{
			...
			_this->m_hAccount = CreateDialogParam(hAppInstance, 
				MAKEINTRESOURCE(IDD_XMPP_ACCOUNT),
				hwnd,
				(DLGPROC)_this->AccountDlgProc,
				(LONG)_this);
			EnableThemeDialogTexture(_this->m_hAccount, ETDT_ENABLETAB);
			...
			_this->m_hShared = CreateDialogParam(hAppInstance, 
				...
				(LONG)_this);
			EnableThemeDialogTexture(_this->m_hShared, ETDT_ENABLETAB);
			...
			_this->m_hIncoming = CreateDialogParam(hAppInstance,
				...
				(LONG)_this);
			EnableThemeDialogTexture(_this->m_hIncoming, ETDT_ENABLETAB);
			...
			_this->m_hPoll = CreateDialogParam(hAppInstance, 
				...
				(LONG)_this);
			EnableThemeDialogTexture(_this->m_hPoll, ETDT_ENABLETAB);
			...
			_this->m_hQuerySettings = CreateDialogParam(hAppInstance, 
				...
				(LONG)_this);
			EnableThemeDialogTexture(_this->m_hQuerySettings, ETDT_ENABLETAB);
			...
			_this->m_hAdministration = CreateDialogParam(hAppInstance, 
				...
				(LONG)_this);
			EnableThemeDialogTexture(_this->m_hAdministration, ETDT_ENABLETAB);
			...
			SetWindowPos(_this->m_hAccount, HWND_TOP, rc.left, rc.top,
			...
			if (_this->m_usersettings) {
				SetWindowText(hwnd, "RemoteVNC Server Current User Properties");
			} else {
				SetWindowText(hwnd, "RemoteVNC Server Default Local System Properties");
			}						
	...
	case WM_COMMAND:
		...
		case IDC_APPLY:
			...
			SendMessage(_this->m_hAccount, WM_COMMAND, IDC_APPLY,0);
...
BOOL CALLBACK vncProperties::AccountDlgProc(HWND hwnd, UINT uMsg,
												  WPARAM wParam, LPARAM lParam)
{
	// We use the dialog-box's USERDATA to store a _this pointer
	// This is set only once WM_INITDIALOG has been recieved, though!
	...
	case WM_INITDIALOG:
		{
			...
			SetWindowLong(hwnd, GWL_USERDATA, lParam);
			vncProperties *_this = (vncProperties *) lParam;
			_this->m_AccountControls = new AccountControls(hwnd, _this->m_server);
			return 0;
		}
	case WM_HELP:	
		VNCHelp::Popup(lParam);
    ...
    case WM_COMMAND:		
		...
		{
		case IDC_APPLY:
			_this->m_AccountControls->Apply();
			return TRUE;
		}
		return 0;
	case WM_DESTROY:
		delete _this->m_AccountControls;      
		_this->m_AccountControls = NULL;  
		_this->m_hAccount = NULL;  
		return 0;
	}
	return 0;	
}
...
vncProperties::LoadUserPrefs(HKEY appkey)
{
	...
	m_server->SetXmppUsername(LoadStdString(appkey, "XmppUsername"));
	m_server->SetXmppPassword(LoadStdString(appkey, "XmppPassword"));
...
void 
vncProperties::SaveString(HKEY key, const char *entry_name, std::string value)
{
	RegSetValueEx(key, entry_name, NULL, REG_SZ, (CONST BYTE *)value.c_str(), value.size()+1);			
}

std::string
vncProperties::LoadStdString(HKEY key, const char *entry_name)
{
	TCHAR buf[256];
	DWORD buffersize = 256;
	DWORD valtype;
	if (RegQueryValueEx(key, entry_name, NULL, &valtype, 
...
vncProperties::SaveUserPrefs(HKEY appkey)
{
	...
	// Save the XMPP settings.
	SaveString(appkey, "XmppUsername", m_server->GetXmppUsername());
	SaveString(appkey, "XmppPassword", m_server->GetXmppPassword());
