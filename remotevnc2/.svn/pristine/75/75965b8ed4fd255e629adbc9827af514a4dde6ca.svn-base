...
BOOL CALLBACK SessionDialog::SessDlgProc(  HWND hwnd,  UINT uMsg,  WPARAM wParam, LPARAM lParam ) 
{
	...
	case WM_INITDIALOG: 
      ...
      CentreWindow(hwnd);
			...
			// Login link.
			Static_SetText(GetDlgItem(hwnd, IDC_LOGGED_IN_NAME), _this->m_cc->m_logged_in_as.c_str());
			ConvertStaticToHyperlink(hwnd, IDC_CHANGE_LOGIN_LINK);

			// Load connection history to the combo box.
			VncRosterProxy *roster = _this->m_cc->GetRoster();
			for (int i = 0; i < roster->Length(); i++) {
				std::string name = roster->GetName(i);
				if (name.empty()) {
					continue;
				}
				SendMessage(hcombo, CB_INSERTSTRING, (WPARAM)0, (LPARAM)name.c_str());
			}
			delete roster;

			Button_SetCheck(GetDlgItem(hwnd, IDC_CUSTOM_RADIO), BST_CHECKED);
	...
	case WM_COMMAND:		
		...
		case IDC_CHANGE_LOGIN_LINK:
			switch (HIWORD(wParam)) {
			case STN_CLICKED:
				/* Force the login info to be changed. */
				_this->m_cc->DoXmppLogin(true);
				break;
			}
			break;
		...
		case IDC_OK:       
			{
				...
				VncRosterProxy *roster = _this->m_cc->GetRoster();
				RosterEntry entry = roster->GetByName(TCHARToString(display));
				StringToTCHAR(entry.jid.Str(), (TCHAR*)jid, 256);
				delete roster;

				_tcscpy(_this->m_cc->m_host_jid, jid);
				_tcscpy(_this->m_cc->m_host, display);
