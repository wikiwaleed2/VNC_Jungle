...
vncService::PostShareAll()
{
	...
	{
		MessageBox(NULL, "No existing instance of RemoteVNC could be contacted", szAppName, MB_ICONEXCLAMATION | MB_OK);
		return FALSE;
	}
...
// Executable name
#define VNCAPPNAME            "remotevnc"

// Internal service name
#define VNCSERVICENAME        "remotevnc"

// Displayed service name
#define VNCSERVICEDISPLAYNAME "RemoteVNC Server"
...
vncService::InstallService(BOOL silent)
{
    ...
    if (GetModuleFileName(NULL, path, pathlength-(strlen(winvncRunService)+2)) == 0) {
		if (!silent) {
			MessageBox(NULL, "Unable to install RemoteVNC service", szAppName, MB_ICONEXCLAMATION | MB_OK);
		}
		return 0;
    }
	...
	case VER_PLATFORM_WIN32_WINDOWS:
		{
			...
			{
				if (!silent) {
					MessageBox(NULL, "The SCM could not be contacted - the RemoteVNC service was not installed", szAppName, MB_ICONEXCLAMATION | MB_OK);
				}
				break;
			}
			...
			if (RegSetValueEx(runservices, szAppName, 0, REG_SZ, (unsigned char *)servicecmd, strlen(servicecmd)+1) != ERROR_SUCCESS)
			{
				...
				if (!silent) {
					MessageBox(NULL, "The RemoteVNC service could not be registered", szAppName, MB_ICONEXCLAMATION | MB_OK);
				}
				break;
			}
			...
			{
				if (!silent) {
					MessageBox(NULL, "The RemoteVNC service failed to start",
	...
	case VER_PLATFORM_WIN32_NT:
		{
			...
			if (hsrvmanager == NULL)
			{
				if (!silent) {
					MessageBox(NULL,
						"The Service Control Manager could not be contacted - the RemoteVNC service was not registered",
						szAppName,
			...
			if (hservice == NULL)
			{
				...
				if (!silent) {
					if (error == ERROR_SERVICE_EXISTS) {
						MessageBox(NULL,
							"The RemoteVNC service is already registered",
							szAppName,
							MB_ICONEXCLAMATION | MB_OK);
					} else {
						MessageBox(NULL,
							"The RemoteVNC service could not be registered",
							szAppName,
							MB_ICONEXCLAMATION | MB_OK);
					}
				}
			...
			if (!silent) {
				vncTimedMsgBox::Do(
					"The RemoteVNC service was successfully registered\n"
	...
	case VER_PLATFORM_WIN32_WINDOWS:
		{
			...
			{
				if (!silent) {
					MessageBox(NULL, "The Service Control Manager could not be contacted - the RemoteVNC service was not unregistered", szAppName, MB_ICONEXCLAMATION | MB_OK);
				}
			}
			else
			{
				...
				if (RegDeleteValue(runservices, szAppName) != ERROR_SUCCESS)
				{
					RegCloseKey(runservices);
					if (!silent) {
						MessageBox(NULL, "The RemoteVNC service could not be unregistered", szAppName, MB_ICONEXCLAMATION | MB_OK);
					}
				}
			...
			if (!KillRunningCopy())
			{
				if (!silent) {
					MessageBox(NULL,
						"The RemoteVNC service could not be contacted",
						szAppName,
						MB_ICONEXCLAMATION | MB_OK);
				}
				break;
			...
			if (!silent) {
				vncTimedMsgBox::Do("The RemoteVNC service has been unregistered", szAppName, MB_ICONINFORMATION | MB_OK);
			}
		}
		break;
				...
				if (hservice != NULL)
				{
					...
					if (ControlService(hservice, SERVICE_CONTROL_STOP, &status))
					{
						...
						if (status.dwCurrentState != SERVICE_STOPPED) {
							if (!silent) {
								MessageBox(NULL, "The RemoteVNC service could not be stopped", szAppName, MB_ICONEXCLAMATION | MB_OK);
							}
						}
					}
					...
					if (DeleteService(hservice)) {
						...
						if (error == ERROR_SERVICE_MARKED_FOR_DELETE) {
							if (!silent)
								MessageBox(NULL, "The RemoteVNC service is already marked to be unregistered", szAppName, MB_ICONEXCLAMATION | MB_OK);
						} else {
							if (!silent)
								MessageBox(NULL, "The RemoteVNC service could not be unregistered", szAppName, MB_ICONEXCLAMATION | MB_OK);
						}
					...
					MessageBox(NULL, "The RemoteVNC service could not be found", szAppName, MB_ICONEXCLAMATION | MB_OK);
